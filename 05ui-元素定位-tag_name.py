...
# 使用tag name定位 输入用户名：admin
driver.find_element("tag name", "input").send_keys("18300000000")
# 使用tag name定位 输入密码：password
# find_elements 返回的是一个列表，列表中存放了所有符合条件的元素对象。可以使用下标来获取指定的元素对象。
driver.find_elements("tag name", "input")[1].send_keys("2313231")
