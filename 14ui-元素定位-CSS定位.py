...
# 4.暂停1秒 
time.sleep(1)
# 使用CSS定位方式中的id选择器定位用户名输入框输入admin
driver.find_element("css selector","#userA").send_keys("admin")
time.sleep(1)
# 使用CSS定位方式中的属性选择器定位密码输入框输入123456
driver.find_element("css selector","[name='passwordA']").send_keys("123456")
time.sleep(1)
# 使用CSS定位方式中的class选择器定位电话号码输入框输入13800138000
driver.find_element("css selector",".telA").send_keys("13800138000")
time.sleep(1)
# 使用CSS定位方式中的元素选择器定位注册按钮，并点击
